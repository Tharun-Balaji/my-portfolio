...
## Commit Message Structure (Short + Description)

Use this format for every migration commit:
...
- `chore`: tooling, config, CI wiring
- `refactor`: code migration without behavior change
- `test`: test files, coverage, test setup
...
```text
chore: add eslint and prettier baseline

Set up consistent linting/formatting before TS migration to reduce noisy diffs.
Added flat ESLint config for Astro + React and Prettier config with Astro plugin.
Validated using: npm run lint && npm run format:check.
```

```text
refactor: convert navbar from jsx to tsx

Migrated Navbar to TypeScript to remove implicit any and prepare strict typing.
Renamed Navbar.jsx to Navbar.tsx and updated imports in index.astro.
Validated using: npm run astro check && npm run build.
```
